...
use App\Repositories\Frontend\Access\User\ProfileRepository;
...
class UserRepository extends BaseRepository
{
     ...
     */
    public function __construct(RoleRepository $role, ProfileRepository $profile)
    {
        $this->role     = $role;
    ...
    public function updateProfile($id, $input)
    {
        ...
        if ($user->canChangeEmail()) {
            ...
            if ($user->email != $input['email']) {
                ...
                $this->updateUserProfile($input, $user);
     ...
     * @return User $user
     */
    public function updateUserProfile($input, $user)
    {
        DB::beginTransaction();
        try {
            $user->save();
            $this->profile->store($input, $user->profile);
            DB::commit();
            return $user;
        } catch (\Exception $e) {
            DB::rollback();
            throw new GeneralException('It appears that an error occured during the execution. Will be reporting this incident to prevent this issue again.');
        }
    }
