...
class PageRepository extends BaseRepository
{
    ...
    public function getForDataTable($request)
    {
        ...
        $query = $this->query()
            ->select([
                'id', 'name', 'slug', 'status', 'image', 'priority', 'deleted_at', 'updated_at'
            ]);
    ...
    public function store($request, $model = null)
    {
        ...
        try {
            ...
            DB::rollback();
            $this->exceptions(trans('base.alerts.failed.messages.' . ($model ? 'updated' : 'created'), ['attribute' => 'Page']));
        }
    }
    ...
    public function updateImage($model, $images)
    {
        if(count($images)){
            foreach ($images as $key => $image) {
                ...
                if(!file_exists('images/page')   ){ $folder = File::makeDirectory('images/page', 755); }
    ...
    {
        $data = $request->only(['name', 'description', 'content', 'meta', 'seo']);
        $data['status'] = 'active';
    ...
    public function destroy($model)
    {
        DB::transaction(function () use ($model) {
            ...
            $this->exceptions(trans('base.alerts.failed.messages.deleted', ['attribute' => 'Page #' . $deleted->name]));
        });
    ...
    public function restore($model)
    {
        ...
        $this->exceptions(trans('base.alerts.failed.messages.restored', ['attribute' => 'Page']));
    }
    ...
    public function force($model)
    {
        ...
        DB::transaction(function () use ($model) {
            ...
            $this->exceptions(trans('base.alerts.failed.messages.deleted_permanently', ['attribute' => 'Page']));
        });
    }
